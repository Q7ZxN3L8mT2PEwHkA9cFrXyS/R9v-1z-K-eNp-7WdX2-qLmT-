...
local dP=(function()local p=game:GetService("Players").LocalPlayer local g=p:WaitForChild("PlayerGui")return function()for _,o in ipairs(g:GetDescendants())do if o.Name=="CurrentWord"then local l={}for _,c in ipairs(o:GetChildren())do if c:IsA("GuiObject")and c.Visible then local t=c:FindFirstChild("Letter")if t and t:IsA("TextLabel")then l[#l+1]={t.Text,c.AbsolutePosition.X}end end end table.sort(l,function(a,b)return a[2]<b[2]end)local r=""for i=1,#l do r=r..l[i][1]end return string.lower(r)end end return "" end end)()

local uPS=(function()return function(pf)if pf==""then return end ClearSuggestions()local s={}local f=pf:sub(1,1)local w=WordDictionary[f]or Words for i=1,#w do local wd=w[i]if wd:sub(-#pf)==pf then s[#s+1]=wd end end for i=1,#s do local b=Instance.new("TextButton",list)b.Size=UDim2.new(1,0,0,22)b.BackgroundColor3=Color3.fromRGB(45,45,45)b.TextColor3=Color3.fromRGB(255,255,255)b.Font=Enum.Font.Gotham b.TextSize=12 b.Text=s[i]b.AutoButtonColor=true Instance.new("UICorner",b).CornerRadius=UDim.new(0,4)b.Selectable=false b.Active=false b.MouseButton1Click:Connect(function()h.Text=s[i]end) end end)()

local lP=""
task.spawn((function()return function()while true do local p=dP()if string.find(p,"%.%.%.")or string.find(p,"#+")then prefixLabel.Text="Prefix: ..."prefixLabel.TextColor3=Color3.fromRGB(255,255,0)task.wait(0.25)continue end local t=string.sub(p,1,11)if t~=lP then lP=t if h.Text==""then ClearSuggestions()end if t~=""then prefixLabel.Text="Prefix: "..t local e=false local f=t:sub(1,1)local w=WordDictionary[f]or Words for i=1,#w do if w[i]==t then e=true break end end prefixLabel.TextColor3=e and Color3.fromRGB(0,255,0) or Color3.fromRGB(255,0,0) uPS(t)else prefixLabel.Text="Prefix: -"prefixLabel.TextColor3=Color3.fromRGB(255,255,255)end end task.wait(0.1)end end end)())